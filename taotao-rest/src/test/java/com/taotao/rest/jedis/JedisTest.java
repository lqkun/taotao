...
private static final Logger LOGGER = LoggerFactory.getLogger(JedisTest.class);
	...
	public void testJedisSingle() {
		...
		jedis.set("lqkun", "jedis test");
		String string = jedis.get("lqkun");
	...
	public void testJedisPool() {
		//创建jedis连接池
		JedisPool pool = new JedisPool("192.168.21.129", 6379);
		//从连接池中获得Jedis对象
	...
	public void testJedisCluster() {
		...
		nodes.add(new HostAndPort("192.168.21.129", 7001));
		nodes.add(new HostAndPort("192.168.21.129", 7002));
		...
		nodes.add(new HostAndPort("192.168.21.129", 7006));
		LOGGER.info("创建一个JedisCluster对象");
		JedisCluster cluster = new JedisCluster(nodes);
		LOGGER.debug("设置key1的值为test");
		cluster.set("key2", "test2");
	...
	public void testSpringJedisCluster() {
		...
		jedisCluster.set("lqkun", "it is a test!");
		String string = jedisCluster.get("lqkun");
		System.out.println(string);
		jedisCluster.close();
	}
}
